...
#undef update_param
    ...
    param->vad_enabled = PJ_FALSE; // VAD KVT
    }
    /* else { // VAD KVT
	param->flags |= PJMEDIA_AUD_DEV_CAP_VAD;
...
static pj_status_t open_snd_dev(pjmedia_snd_port_param *param)
{
    ...
    //pjmedia_splitcomb_create(pjsua_var.snd_pool, param->base.clock_rate, 1, param->base.samples_per_frame,
    //                         param->base.bits_per_sample, 0, &splitcomb);
    //pjmedia_splitcomb_create_rev_channel(pjsua_var.snd_pool, splitcomb, 0, 0, &rev);
    // KVT
    ...
    // KVT
    //pjmedia_master_port_create(pjsua_var.snd_pool, rev, conf_port, 0, &m);
